...
- `.cursor/skills/linear-project-orchestration/` — оркестрация Linear-проектов (MCP): очередь, статусы, отчёты-документы
- `src/atman/term.py` — общий слой **Rich** для CLI и демо (панели, таблицы, стилизованные сообщения)

### Linear — задачи, проекты, отчёты (обязательно)

**Канон:** `docs/development/DEVELOPMENT_STANDARD.md` **§30**. Детальный пошаговый сценарий:
[`.cursor/skills/linear-project-orchestration/SKILL.md`](.cursor/skills/linear-project-orchestration/SKILL.md).

**Плагин:** Linear MCP в Cursor (`save_issue`, `save_project`, `save_document`, …). Без авторизации MCP в **Settings → MCP** — не начинать работу по Linear.

#### Статусы issue (железное правило)

1. Взял issue → **сразу** `save_issue` → **In Progress** (до кода и до чтения репо под задачу).
2. Закончил работу → **сразу** `save_issue` → **Done** (не после PR, не «потом»).

❌ Начинать без In Progress · ❌ Закрывать без Done · ❌ Откладывать смену статуса.

#### Работа по проекту

Когда пользователь даёт **Linear project** (имя / slug / id) или просит «работай по проекту X»:

1. Загрузить skill `linear-project-orchestration` (или следовать §30).
2. `list_issues` + `blockedBy` → **ready queue** (нет открытых блокеров).
3. Старт проекта: `save_project` → активный state.
4. Брать issues по очереди (или параллель до 2–3 **только** ready и **только** с явного разрешения пользователя).
5. На каждый закрытый issue: **Linear Document** (completion report) + комментарий на issue — шаблон в skill `templates/issue-completion-report.md`.
6. Когда проект завершён: **closure document** на project со ссылками на все issue-отчёты (`templates/project-closure.md`) + `save_project` → завершённый state.

Не менять issues/projects в Linear без явного запроса пользователя.

**Notion** (epic import) — см. `CLAUDE.md`; day-to-day исполнение — Linear §30.

### PR / CI / Review — железное правило

**Не зависит от контекста, ситуации, приоритета, scope или происхождения проблемы.**

После создания PR (и пока он не смержен):

1. **Любое замечание ревью-бота** (Devin `devin-ai-integration[bot]`, Seer `Seer Code Review`/`seer[bot]`, Sentry `sentry[bot]`, Copilot, etc.) → **фиксить**. Не спрашивать «fix or skip», не аргументировать «pre-existing», «out of scope», «not a regression», «informational only». Если бот указал на проблему — она фиксится в этом же PR.
2. **Любая красная CI-проверка** → **фиксить**. Не игнорировать «известные flaky тесты»: они либо стабилизируются, либо чинятся.
3. **Любой merge-конфликт** → **разрешать**. Rebase / merge с main, без `--hard` сбросов чужой работы.
4. Цикл: каждое webhook-уведомление → расследование → фикс → коммит → пуш → новые проверки. Повторять до тишины.
5. **Дождаться всех известных ревью-ботов перед мержем.** Каждый PR проходит:
   - **Devin** — review threads.
   - **Seer Code Review** — check_run с `details_url=sentry.io`, может оставлять inline comments.
   - **Sentry** — bug prediction comments.
   - **Copilot** — если включён.
   - Плюс штатный CI (`pip-audit`, `Codemap staleness check`, `Lint, typecheck, security, tests`, …).

   **Пока хоть один check висит `in_progress` / `queued` / `pending` или известный ревью-бот не оставил финальный вердикт — НЕ мержить.**
6. **Когда после очередного раунда checks ноль замечаний / ошибок / конфликтов И все боты закончили** → если draft, снять draft и **смержить**. Не ждать «может ещё что-то прилетит» сверх известных ботов.

❌ Спрашивать «фиксить или пропустить» по замечанию бота · ❌ Оставлять CI-ошибки на потом · ❌ Оставлять конфликты на пользователя · ❌ Висеть в draft после полностью зелёных checks · ❌ Мержить пока хоть один known review-бот ещё работает

Единственное исключение: явная инструкция пользователя «не трогай / оставь / не мержи». Иначе — работа автоматическая до merge.
