...
- **Dependency audit**: `pip-audit` (in CI — scoped to project deps only)
- **CI**: GitHub Actions runs all checks on every PR to `main` (see `.github/workflows/ci.yml`)
...
- CI автоматически проверяет все пункты на каждый PR (`.github/workflows/ci.yml`)
...
- CI запускается автоматически на каждый PR к `main`.
