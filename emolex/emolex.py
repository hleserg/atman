...
RU_AMPLIFIERS = {
    "очень": 1.5, "крайне": 1.7, "чрезвычайно": 1.7, "безумно": 1.6,
    "страшно": 1.5, "сильно": 1.4, "жутко": 1.5, "чертовски": 1.5,
    "дико": 1.5, "безмерно": 1.6, "абсолютно": 1.5, "совершенно": 1.4,
    "полностью": 1.3, "невероятно": 1.6, "ужасающе": 1.7, "ужасно": 1.5,
    "поразительно": 1.4, "максимально": 1.5, "глубоко": 1.4,
    "невыносимо": 1.6, "столько": 1.3, "так": 1.3, "очень-очень": 1.8,
    "донельзя": 1.6, "просто": 1.2,
}

RU_WEAKENERS = {
    "чуть": 0.5, "немного": 0.6, "слегка": 0.5, "едва": 0.4,
    "чуточку": 0.4, "малость": 0.5, "несколько": 0.7, "слабо": 0.5,
    "слегонца": 0.5, "капельку": 0.5,
}

RU_NEGATORS = {"не", "ни", "нет", "никогда", "никак", "ничуть", "нисколько"}

EN_AMPLIFIERS = {
    "very": 1.5, "extremely": 1.7, "really": 1.4, "so": 1.3,
    "super": 1.4, "incredibly": 1.6, "terribly": 1.5, "awfully": 1.5,
    "absolutely": 1.5, "completely": 1.4, "utterly": 1.6, "totally": 1.4,
    "highly": 1.4, "deeply": 1.4, "intensely": 1.5, "exceptionally": 1.5,
    "remarkably": 1.4, "particularly": 1.3, "quite": 1.2, "too": 1.3,
}

EN_WEAKENERS = {
    "slightly": 0.5, "somewhat": 0.6, "kinda": 0.6, "kind": 0.6,
    "barely": 0.4, "hardly": 0.4, "little": 0.6, "bit": 0.7,
    "mildly": 0.5, "marginally": 0.5, "rather": 0.7,
}

EN_NEGATORS = {"not", "never", "no", "none", "nothing", "neither", "nor",
               "nobody", "without", "cannot", "cant", "wont", "dont", "doesnt",
               "didnt", "isnt", "arent", "wasnt", "werent", "havent", "hasnt"}
...
def load_lexicons(path_ru: Path | str | None = None,
                   path_en: Path | str | None = None) -> None:
...
def emotion_score(text: str, lang: str = "ru", window: int = 3) -> dict:
    ...
    if lang not in _LEXICONS:
        raise ValueError(f"Словарь для языка '{lang}' не загружен. "
                         f"Запусти build_lexicons.py")
...
def dominant_emotion(score: dict) -> tuple[str, float]:
    ...
    candidates = {k: score[k] for k in EMOTION_KEYS
                  if k not in {"positive", "negative"} and k in score}
