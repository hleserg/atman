...
> Источники истины: `migrations/versions/0001–0014`, `eval/migrations/versions/0010–0040`
...
```
public.*          — общие таблицы (факты, рефлексии, очереди)
                    Изоляция: Row-Level Security по session var atman.current_agent
                    
agent_{N}.*       — приватные схемы агентов (сессии, идентичность, моменты, сущности)
...
### `public.reflections` — рефлексии

Хранилище результатов рефлексии (micro/daily/deep). **RLS: включён, FORCED.**

| Колонка | Тип | Описание |
|---------|-----|----------|
| `id` | BIGSERIAL PK | |
| `agent_id` | UUID NOT NULL | **RLS-поле** |
| `level` | `reflection_level` | `micro` (после сессии) / `daily` (конец дня) / `deep` (плановая глубокая) |
| `created_at` | TIMESTAMPTZ | |
| `session_id` | UUID → sessions | Только для `level='micro'` |
| `period_start` | TIMESTAMPTZ | Только для `daily`/`deep` |
| `period_end` | TIMESTAMPTZ | Только для `daily`/`deep` |
| `content` | TEXT NOT NULL | Полный текст рефлексии |
| `summary` | TEXT | Краткое резюме |
| `experience_refs` | UUID[] | UUID ключевых моментов, охваченных рефлексией |
| `reframing_note_ids` | UUID[] | UUID записей переосмысления, созданных рефлексией |
| `model_provider` | TEXT | LLM-провайдер (ollama, anthropic, ...) |
| `model_name` | TEXT | Конкретная модель (qwen3.5:9b, ...) |
| `schema_version` | INT | Версия схемы (текущая: 1) |
| `metadata` | JSONB | |

**Индексы:** `(agent_id, created_at DESC)`, `(level, created_at DESC)`, GIN на `experience_refs`, `session_id`

---

### `public.self_applied_changes` — аудит самостоятельных изменений

Каждое изменение идентичности/нарратива, применённое рефлексией, пишется сюда. **RLS: отсутствует** (single-agent workspace, проверка на уровне сервиса).

| Колонка | Тип | Описание |
|---------|-----|----------|
| `id` | UUID PK | |
| `applied_at` | TIMESTAMPTZ | |
| `agent_id` | UUID | Nullable: NULL для нарративных изменений |
| `actor` | TEXT | `reflection_daily` / `reflection_deep` / `human_via_reflection_review` |
| `reflection_event_id` | UUID NOT NULL | Ссылка на событие рефлексии |
| `target_kind` | TEXT | Что изменилось: `identity_core_value`, `identity_principle`, `identity_habit`, `identity_goal`, `identity_open_question`, `identity_self_description`, `narrative_core_layer`, `narrative_recent_layer` |
| `target_ref` | TEXT | Идентификатор конкретного элемента |
| `before_snapshot` | JSONB | Состояние до изменения |
| `after_snapshot` | JSONB | Состояние после изменения |
| `rationale` | TEXT | Обоснование изменения |
| `confidence_self_assessment` | TEXT | Самооценка уверенности рефлексии |
| `based_on_moment_ids` | UUID[] | Ключевые моменты, на которых основано решение |
| `reverted_at` | TIMESTAMPTZ | Дата отката (если откатили) |
| `reverted_reason` | TEXT | Причина отката |
| `reverted_by_change_id` | UUID → self_applied_changes | FK на строку-откат |

---

### `public.pending_human_review` — очередь на проверку человеком

Рефлексия пишет сюда, когда недостаточно уверена для самостоятельного изменения. **RLS: отсутствует.**

| Колонка | Тип | Описание |
|---------|-----|----------|
| `id` | UUID PK | |
| `created_at` | TIMESTAMPTZ | |
| `created_by` | TEXT | Кто создал (имя компонента рефлексии) |
| `reflection_event_id` | UUID | Ссылка на рефлексию |
| `kind` | TEXT | `identity_change_doubt` / `narrative_change_doubt` / `high_salience_judgement` |
| `question` | TEXT | Вопрос к человеку |
| `context` | JSONB | Контекст для принятия решения |
| `priority` | TEXT | `normal` / `high` |
| `resolved_at` | TIMESTAMPTZ | Дата резолюции |
| `resolution` | TEXT | `accepted` / `rejected` / `modified` / `dismissed` / NULL |
| `resolution_note` | TEXT | Комментарий к решению |
| `applied_change_id` | UUID → self_applied_changes | Примечённое изменение (если accepted/modified) |

**Индекс:** `(priority, created_at) WHERE resolved_at IS NULL` — для эффективного получения очереди
...
| `formed_in_reflection_id` | UUID | Soft ref → public.reflections |
...
| Колонка | Тип | Описание |
|---------|-----|----------|
| `reflection_id` | BIGINT → public.reflections | CASCADE DELETE |
| `entity_id` | UUID → entities | RESTRICT |
| `agent_id` | UUID | |
...
### Граф знаний
`entities` → `entity_aliases` (имена) → `entity_relations` (типизированные рёбра) → `entity_stance` (позиция агента). Extraction: mrebel (авто) или ручная разметка.

### Human-in-the-loop
`pending_human_review` → резолюция человека → `self_applied_changes` (аудит) → возможный откат через `reverted_by_change_id`.
