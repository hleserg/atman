...
| `skills/cli.py` | CLI навыков (точка входа `atman-skills`) — использует Rich через `atman.term` (`console`, `print_ok`/`print_err`/`print_warn`/`print_help_text`, `Panel`, `Table`); без raw `print()` по AGENTS.md | `list`, `show`, `disable`, `enable`, `pin`, `unpin`, `archive`, `inspect-invocations` (PR #604: таблица из `list_invocations_by_skill`), `capture-manual` (PR #604), `force-revise`, `install-external` (HLE-38), `revise` (HLE-40); read-only при `enabled=false` |
