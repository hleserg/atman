...
- Перед началом работы агент обязан прочитать корневой `AGENTS.md`.
...
2. Добавить ссылку на корневой `AGENTS.md` как обязательные правила разработки.
3. Добавить ссылку на `docs/architecture/SYSTEM.md` как архитектурный источник.
4. Попросить не менять соседние пакеты без необходимости.
5. Требовать PR с командами запуска и тестов в описании.

Минимальная формулировка для GitHub issue:

```text
Перед работой обязательно прочитай AGENTS.md, docs/architecture/SYSTEM.md и docs/development/work-packages/<package>.md.
Следуй AGENTS.md как обязательным правилам проекта.
Реализуй только указанный work package как самостоятельный runnable-срез.
```
