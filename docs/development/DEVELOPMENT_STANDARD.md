...
1. **Человекочитаемое описание фичи** — что изменилось и какие инварианты
   важны. Размещение: `docs/development/work-packages/` (предпочтительно) или
   согласованный walkthrough в корне (например `README_EXPERIENCE_STORE.md` для
   Experience Store).
...
**Исключение (agent-facing walkthrough):** краткий обзор крупной реализованной
фичи в духе `README_EXPERIENCE_STORE.md` допускается в корне, если он служит
единой точкой входа для демонстрации (см. Definition of Demo). Для новых фич
предпочтительно вести полное ТЗ в `docs/development/work-packages/`, а в корне
при необходимости оставлять только отсылку.
