...
## 22. Definition of Done для любого пакета
...
## 23. ADR: когда нужен архитектурный документ
...
## 26. Checklist перед началом новой задачи
...
- если задача из **Linear** — статусы In Progress / Done и отчёт (§30) запланированы?

## 27. Карта системы и тестовое покрытие
...
## 28. Принцип безопасности смысла
...
## 29. Документация: когда создавать и что писать
...
## 30. Linear: обязательный workflow для агентов

Linear — канонический трекер задач для людей и агентов (плагин **Linear MCP** в Cursor).
Notion (`CLAUDE.md`, issues page) остаётся для epic-импорта; **исполнение задачи** и
статусы — в Linear, если задача привязана к Linear issue или проекту.

Полная пошаговая оркестрация проекта (очередь по блокерам, параллель, отчёты-документы):
skill [`.cursor/skills/linear-project-orchestration/SKILL.md`](../../.cursor/skills/linear-project-orchestration/SKILL.md)
и шаблоны в `templates/` рядом с skill.

### 30.1 Железные правила статусов (issue)

Без исключений, для **каждого** Linear issue, над которым ведётся работа:

1. **До** любой работы в репозитории → `save_issue` → состояние **In Progress**
   (точное имя — из `list_issue_statuses` команды issue).
2. Работа в коде/доках по `AGENTS.md` / DoD.
3. **Сразу** по завершении работы (не «после PR», не «потом») → `save_issue` → **Done**.

Запрещено: начинать без In Progress; заканчивать без Done; откладывать смену статуса.

Инструменты: Linear MCP (`save_issue`, `save_project`, `save_document`, `save_comment`, …).

### 30.2 Оркестрация проекта (когда пользователь дал проект)

Если пользователь просит работать **по Linear-проекту** (имя, slug или id):

1. **Discover:** `get_project`, `list_issues` по проекту, при необходимости `get_issue` для `blockedBy` / `blocks`.
2. **Ready queue:** issue готов, если не в терминальном статусе и все блокеры уже **Done**.
   Сортировка: приоритет, затем давность обновления.
3. **Старт проекта:** при первом входе в работу — `save_project` → активный `state` команды
   (уточнить через `get_project` / конвенцию workspace).
4. **Взять задачу:** первый ready (или параллель — см. 30.3) → In Progress → assignee `"me"` при необходимости.
5. **Закрытие issue:** Done → **отчёт** (30.4) → следующий ready.
6. **Закрытие проекта:** когда все целевые issues Done → closure-document на проект (30.4) → `save_project` → завершённый `state`.

Не менять Linear без явного запроса пользователя («посмотри Linear» ≠ «возьми проект X»).

### 30.3 Параллельная работа

Только если пользователь **явно** разрешил параллель или «всё без блокеров параллельно»:

- Параллелить только issues из **ready queue** без открытых `blockedBy`.
- По умолчанию не более **2** одновременных issues; до **3** — только по просьбе пользователя.
- Не параллелить issues с пересечением по одним и тем же файлам без согласия.
- На каждый issue — отдельный subagent/изолированный прогон с теми же правилами статусов и отчёта.

### 30.4 Отчёты в Linear Documents (канон)

По завершении issue (**после** Done):

1. `save_document` с родителем `issue` — markdown по шаблону
   `.cursor/skills/linear-project-orchestration/templates/issue-completion-report.md`
   (Summary, Changes, Verification, Follow-ups, Links).
2. `save_comment` на issue со ссылкой на документ.
3. При наличии URL — `save_issue` `links` с заголовком «Completion report».

Локальная копия в `reports/sessions/` — **опциональна**, только если пользователь просит git-артефакт;
канон для handoff — **Linear Document** на issue.

По завершении **всего проекта**:

1. `save_document` с родителем `project` — шаблон `project-closure.md` в том же каталоге skill:
   итог, таблица ссылок на все issue completion reports.
2. `save_project` — завершённый `state`, при необходимости краткий `summary`.

### 30.5 Блокировки и сбои

| Ситуация | Действие |
|----------|----------|
| Нет ready issues, есть блокеры | Сообщить цепочку блокеров; не брать заблокированное |
| MCP не авторизован | Остановиться; пользователь подключает Linear в Settings → MCP |
| Работа заблокирована | `save_comment` на issue; статус In Progress или team «Blocked»; **не** Done |
| Задача не из Linear | §30 не применяется |

### 30.6 Checklist (дополнение к §26)

Если задача из Linear, перед merge/завершением агент проверяет:

- [ ] In Progress выставлен **до** работы
- [ ] Done выставлен **сразу** по завершении
- [ ] Completion report (Linear Document) на issue
- [ ] При закрытии проекта — closure document + состояние проекта
- [ ] `make check` пройден, если менялся код

## 31. Notion (epic issues)

Epic-issues после генерации сохранять на Notion issues page (см. `CLAUDE.md`).
Исполнение и статусы day-to-day — Linear (§30), если не оговорено иное.
