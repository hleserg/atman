...
| `ecd0eee` | Генерация синтетического русского NER могла перезаписать `eval/data/atman_ner_ru_synth.jsonl` частичным или пустым файлом после malformed download или слишком малого числа сконвертированных строк | исправлено: JSONL из download парсится строго, минимум 1500 строк стал fatal, замена файла выполняется атомарно только после валидации; покрыто `tests/test_generate_synthetic_ru.py` |
