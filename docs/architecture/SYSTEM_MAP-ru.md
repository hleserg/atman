...
|------|----------------|-----------|
| `adapters/memory/in_memory_backend.py` (`InMemoryBackend`) | `FactualMemory` | без персистенса; `search()` возвращает результаты, отсортированные по salience DESC (чтобы обрезка по `limit` сохраняла самые важные факты) |
| `adapters/memory/file_backend.py` (`FileBackend`) | `FactualMemory` | JSONL + file locking; `search()` отсортирован по salience DESC, как у InMemoryBackend |
...
| `adapters/memory/mock_embedding.py` (`MockEmbeddingAdapter`) | `EmbeddingPort` | детерминированные 1024-мерные эмбеддинги на основе SHA-256 seed; без внешних зависимостей; для тестов/CI |
| `adapters/memory/bm25_embedding.py` (`BM25EmbeddingAdapter`) | `EmbeddingPort` | локальные разреженные BM25-векторы через feature hashing фиксированной размерности (Unicode-aware токенизатор); статистика корпуса из `embed_batch`/`embed_with_corpus` переиспользуется последующими вызовами `embed` |
| `adapters/memory/ollama_embedding.py` (`OllamaEmbeddingAdapter`) | `EmbeddingPort` | Ollama HTTP `/api/embeddings`; по умолчанию: `bge-m3`/1024d; env: `EMBEDDING_MODEL`, `EMBEDDING_OLLAMA_HOST` (устаревшие: `OLLAMA_EMBED_MODEL`, `OLLAMA_HOST`); настраиваемый timeout; **opt-2**: per-instance `lru_cache(maxsize=cache_size)` на `embed()` — повторные упоминания сущности пропускают HTTP round-trip; `cache_size=0` отключает; `embedding_cache_info()` возвращает статистику попаданий |
...
| `adapters/memory/in_memory_usage_log.py` (`InMemoryUsageLog`) | `MemoryUsageLog` | in-memory append-only список с фильтрацией по item/usage_type/времени (без вытеснения) |
...
| `adapters/storage/in_memory_state_store.py` (`InMemoryStateStore`) | `StateStore` | полная реализация в памяти с глубоким копированием; **v2**: словарь сессий + самостоятельные key_moments + `store_key_moment` (идемпотентный upsert), `mark_moment_accessed`, `update_moment_structured_markers`, `create_session`/`get_session`/`update_session`/`list_recent_sessions`; сортировка сессий нормализует legacy naive + UTC-aware timestamps |
| `adapters/storage/file_state_store.py` (`FileStateStore`) | `StateStore` | JSON-файлы (опыт + identity + нарратив + eigenstate) + `key_moments.jsonl`; **v2**: поддержка фильтрации `list_key_moments(session_id)`; `update_moment_structured_markers` сохраняет lingvo-маркеры в per-moment файлы, JSONL и любой совпадающий per-session bundle; `get_key_moment` откатывается к legacy JSONL и backfill-ит per-moment index; сортировка сессий нормализует legacy naive + UTC-aware timestamps |
| `adapters/memory/in_memory_entity_registry.py` (`InMemoryEntityRegistry`) | `EntityRegistry` | L1 (точный псевдоним, регистронезависимо) + L2 (косинус на чистом Python ≥ 0.85) + L3 (создание новой); потокобезопасный через Lock; хелперы `clear()`/`count()` для тестов |
| `adapters/memory/postgres_entity_registry.py` (`PostgresEntityRegistry`) | `EntityRegistry` | Те же L1/L2/L3 над `agent_N.entities` + `agent_N.entity_aliases`; `halfvec` косинус для L2; guarded импорт psycopg3 |
...
| `adapters/memory/in_memory_memory_guardian.py` (`InMemoryMemoryGuardian`) | `MemoryGuardian` | scan_orphan_entities + scan_merge_candidates + scan_stale_moments + scan_embedding_gaps + жизненный цикл findings (write/get_unresolved/resolve); **HLE-31** также реализует `scan_quality_metrics` — sliding-window проверки для `affect_detector_silent` (incomplete_coloring rate), `divergence_pattern` (одинаковый divergence_type ≥ N в пределах окна через опциональный `DivergenceEventStore`), `stance_formation_too_fast` (stance сформирован < 24h после опорных моментов через опциональный `EntityStanceStore`). Отсутствие опциональных входов ⇒ под-скан молча возвращает 0 findings. **HLE-32** также реализует inline пост-write проверки: у факта отсутствует `metadata['embedding']` → `embedding_missing`; entity без embedding (кроме value/principle) → `embedding_missing`; KeyMoment с `incomplete_coloring=True` → `affect_detector_silent` (severity `info`). Все inline-проверки делают dedup по `(target_table, target_id, finding_type)` среди unresolved findings |
| `adapters/memory/noop_reranker.py` (`NoOpReranker`) | `MemoryReranker` | passthrough — возвращает кандидатов, отсортированных по существующему score; деплой без модели реранкера |
| `adapters/memory/bge_reranker.py` (`BgeReranker`) | `MemoryReranker` | `BAAI/bge-reranker-v2-m3` через FlagEmbedding; ленивая загрузка; guarded imports; прозрачный fallback на исходный порядок score при ошибке инференса |
...
| `adapters/linguistic/gliner_minilm_adapter.py` (`GLiNERPlusMiniLMAdapter`) | `LinguisticAnalyzer` | GLiNER (`urchade/gliner_multi-v2.1`) + MiniLM NLI (`MoritzLaurer/multilingual-MiniLMv2-L6-mnli-xnli`); ленивая загрузка модели; guarded imports; эвристики расхождения для русского языка; требует `pip install -e ".[linguistic]"`; **opt-2**: session-scoped SHA-256 кэш на `analyze_user_message()` — повторные упоминания пропускают инференс GLiNER+MiniLM; `clear_session_cache()` вызывается runner'ом при завершении сессии |
...
| `adapters/maintenance/postgres_queue.py` (`PostgresMaintenanceQueue`) | `MaintenanceQueue` | SKIP LOCKED `claim_batch` через CTE над `public.maintenance_jobs`; идемпотентность через run_key; psycopg3 |
| `adapters/linguistic/mrebel_adapter.py` (`MRebelRelationAdapter`) | `EntityRelationExtractor` | `Babelscape/mrebel-large` через transformers `text2text-generation`; ленивая загрузка; парсер REBEL-триплетов; guarded imports |
| `adapters/reflection/state_store_session_repository.py` (`StateStoreSessionRepository`) | `SessionRepository` | тонкий адаптер над любым `StateStore` (InMemory / File / Postgres v2); слот `agent_id` по умолчанию в конструкторе для single-agent деплоев + явная трёхаргументная форма для multi-agent реестров; нормализует границы диапазона и legacy naive timestamp сессий/моментов в UTC, предупреждает при насыщении client-side fetch cap; фундамент для R3+R4 (миграция Daily/Deep reflection) |
...
| `observability/sentry_init.py` (HLE-240) | — | канонический `init_observability(level)` / `is_enabled()`; уровни `ATMAN_OBS_LEVEL` (`off` \| `minimal` \| `debug` \| `verbose`); `off` не импортирует `sentry_sdk`; уровни подключают `traces_sampler`, scrubbing, Spotlight, profiling — см. `docs/features/observability/levels.md` |
...
| `observability/spans.py` | — | `ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`, `pipeline_span`, `job_scope` — общие имена op для сканера инструментирования |
...
| `adapters/agent/pending_reviews_context.py` (R11.7) | — | хелпер `format_pending_reviews_block`: priority-first, oldest-first, обрезка контекста |
...
| `adapters/agent/instructions.py` (`build_instructions`, `build_memory_context`, `build_skill_suggestions_section`) | — | `build_instructions`: строит поведенческие правила (как агент использует инструменты, обязательства); identity/narrative перенесены в `build_memory_context()` для доставки через `inject_memory()`; при `memory_injection_mode == "system_prompt"` добавляет `deps.injected_context`; **WP-08 v2**: `_build_pinned_skills_section(deps)` внедряет список закреплённых навыков, когда `deps.skill_manager` не None; `build_skill_suggestions_section(suggestions)` рендерит `## Релевантные навыки` для попаданий `trigger_router` за ход (runner добавляет через `inject_memory`); `_build_self_awareness_section(deps)` внедряет самоописание Atman (память, навыки, рефлексия); ошибки перехватываются → fallback на пустую строку |
