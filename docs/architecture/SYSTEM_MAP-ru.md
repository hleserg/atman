...
|------|----------------|-----------|
| `adapters/memory/in_memory_backend.py` (`InMemoryBackend`) | `FactualMemory` | без персистенса; `search()` возвращает результаты в порядке убывания salience (чтобы усечение по `limit` сохраняло самые важные факты) |
| `adapters/memory/file_backend.py` (`FileBackend`) | `FactualMemory` | JSONL + file locking; `search()` упорядочен по salience DESC, как у InMemoryBackend |
...
| `adapters/memory/mock_embedding.py` (`MockEmbeddingAdapter`) | `EmbeddingPort` | детерминированные SHA-256-seeded 1024-мерные эмбеддинги; без внешних зависимостей; для тестов/CI |
...
| `adapters/storage/in_memory_state_store.py` (`InMemoryStateStore`) | `StateStore` | полная реализация в памяти с глубоким копированием; **v2**: словарь сессий + самостоятельные key moments + `store_key_moment` (идемпотентный upsert), `mark_moment_accessed`, `update_moment_structured_markers`, `create_session`/`get_session`/`update_session`/`list_recent_sessions`; сортировка сессий нормализует legacy naive + UTC-aware timestamp |
...
| `adapters/memory/in_memory_entity_registry.py` (`InMemoryEntityRegistry`) | `EntityRegistry` | L1 (точный псевдоним, регистронезависимо) + L2 (чистый Python, косинус ≥ 0.85) + L3 (создание); потокобезопасный (Lock); хелперы `clear()`/`count()` для тестов |
...
| `adapters/linguistic/gliner_minilm_adapter.py` (`GLiNERPlusMiniLMAdapter`) | `LinguisticAnalyzer` | GLiNER (`urchade/gliner_multi-v2.1`) + MiniLM NLI (`MoritzLaurer/multilingual-MiniLMv2-L6-mnli-xnli`); ленивая загрузка; guarded imports; эвристики расхождения для русского языка; требует `pip install -e ".[linguistic]"`; **opt-2**: session-scoped SHA-256 кэш на `analyze_user_message()` — повторные фразы пропускают инференс GLiNER+MiniLM; `clear_session_cache()` вызывается runner'ом при завершении сессии |
...
| `adapters/linguistic/mrebel_adapter.py` (`MRebelRelationAdapter`) | `EntityRelationExtractor` | `Babelscape/mrebel-large` через transformers `text2text-generation`; ленивая загрузка; парсер триплетов REBEL; guarded imports |
...
| `adapters/agent/instructions.py` (`build_instructions`, `build_memory_context`, `build_skill_suggestions_section`) | — | `build_instructions`: строит поведенческие правила (как агент использует инструменты, обязательства); идентичность/нарратив перенесены в `build_memory_context()` для доставки через `inject_memory()`; при `memory_injection_mode == "system_prompt"` добавляет `deps.injected_context`; **WP-08 v2**: `_build_pinned_skills_section(deps)` инжектирует список закреплённых навыков, когда `deps.skill_manager` не None; `build_skill_suggestions_section(suggestions)` рендерит `## Релевантные навыки` для попаданий `trigger_router` за ход (runner добавляет через `inject_memory`); `_build_self_awareness_section(deps)` инжектирует self-описание Atman (память, навыки, рефлексия); ошибки перехватываются → пустая строка |
