...
| `adapters/observability/sentry.py` | — | opt-in инициализация Sentry SDK через `SENTRY_DSN`; scope-теги (`agent_id`, `session_id`); spans сессии/maintenance/reflection; capture тихих исключений; helpers метрик; `install_slog_breadcrumb_hook()` прокидывает каждый `slog()` в Sentry breadcrumbs, сохраняя уже зарегистрированный display hook (например Rich-консоль live_chat) |
