...
| `core/ports/maintenance_queue.py` | DB-очередь cron-задач технического обслуживания; семантика SKIP LOCKED, идемпотентность через run_key; `validate_enqueue_payload` отклоняет moment-scoped задачи (`mrebel_extract`, `lingvo_enrich`) без `key_moment_id` в payload при постановке в очередь | `MaintenanceQueue` (ABC): `enqueue`, `claim_batch`, `mark_done`, `mark_failed`, `mark_skipped`, `list_jobs`; `validate_enqueue_payload` |
...
<!-- codemap:auto:start section="modules-cli" lang="ru" -->
| Файл | Реализует порт | Поведение |
|------|-----------------|----------|
| `adapters/memory/in_memory_backend.py` (`InMemoryBackend`) | `FactualMemory` | без персистенса; `search()` возвращает результаты, отсортированные по salience DESC (поэтому усечение по `limit` сохраняет самые важные факты) |
| `adapters/memory/file_backend.py` (`FileBackend`) | `FactualMemory` | JSONL + файловая блокировка; `search()` отсортирован по salience DESC как у InMemoryBackend |
...
| `adapters/memory/mock_embedding.py` (`MockEmbeddingAdapter`) | `EmbeddingPort` | детерминированные 1024-мерные эмбеддинги на основе SHA-256; без внешних зависимостей; для тестов/CI |
| `adapters/memory/bm25_embedding.py` (`BM25EmbeddingAdapter`) | `EmbeddingPort` | локальные разреженные BM25-векторы через feature hashing фиксированной размерности (юникод-совместимый токенизатор); статистика корпуса из `embed_batch`/`embed_with_corpus` повторно используется последующими вызовами `embed` |
| `adapters/memory/ollama_embedding.py` (`OllamaEmbeddingAdapter`) | `EmbeddingPort` | Ollama HTTP `/api/embeddings`; по умолчанию: `bge-m3`/1024d; env: `EMBEDDING_MODEL`, `EMBEDDING_OLLAMA_HOST` (устаревшие: `OLLAMA_EMBED_MODEL`, `OLLAMA_HOST`); настраиваемый timeout; **opt-2**: per-instance `lru_cache(maxsize=cache_size)` на `embed()` — повторные упоминания сущности пропускают HTTP round-trip; `cache_size=0` отключает кеш; `embedding_cache_info()` возвращает статистику попаданий |
| `adapters/memory/flag_embedding.py` (`FlagEmbeddingAdapter`) | `EmbeddingPort` | Нативный FlagEmbedding SDK (BGEM3FlagModel) через PyTorch; ленивая загрузка модели (~570МБ в `~/.cache/huggingface/`); поддерживает dense (1024d) + sparse (лексические) + ColBERT через `embed_batch_full()`; настраиваемые FP16, batch_size, max_length, device; не требует внешнего процесса; по умолчанию: `BAAI/bge-m3`; env: `EMBEDDING_FLAG_MODEL`, `EMBEDDING_USE_FP16`, `EMBEDDING_BATCH_SIZE`, `EMBEDDING_MAX_LENGTH`; **opt-2**: per-instance `lru_cache(maxsize=cache_size)` на `embed()` — пропускает инференс модели при повторных текстах; `embedding_cache_info()` для мониторинга |
| `adapters/memory/in_memory_usage_log.py` (`InMemoryUsageLog`) | `MemoryUsageLog` | in-memory append-only список с фильтрацией по item/usage_type/time (без вытеснения) |
| `adapters/storage/in_memory_experience_store.py` (`InMemoryExperienceStore`) | `StateStore` | в памяти (частично: только Опыт; операции KeyMoment/Identity/Narrative выбрасывают `NotImplementedError`) |
| `adapters/storage/jsonl_experience_store.py` (`JsonlExperienceStore`) | `StateStore` | JSONL для Опыта (частично: только Опыт; операции KeyMoment/Identity/Narrative выбрасывают `NotImplementedError`) |
| `adapters/storage/in_memory_state_store.py` (`InMemoryStateStore`) | `StateStore` | полная in-memory реализация с глубокими копиями; **v2**: словарь sessions + отдельные key_moments + `store_key_moment` (идемпотентный upsert), `mark_moment_accessed`, `update_moment_structured_markers`, `create_session`/`get_session`/`update_session`/`list_recent_sessions`; сортировка сессий нормализует устаревшие naive- и UTC-aware-таймстампы |
| `adapters/storage/file_state_store.py` (`FileStateStore`) | `StateStore` | JSON-файлы (Опыт + Идентичность + Нарратив + Айгенстейт) + `key_moments.jsonl`; **v2**: поддержка фильтрации `list_key_moments(session_id)`; `update_moment_structured_markers` сохраняет lingvo-маркеры в per-moment файлы, JSONL и любой подходящий per-session bundle; `get_key_moment` откатывается к legacy JSONL и backfill'ит per-moment индекс; сортировка сессий нормализует устаревшие naive- и UTC-aware-таймстампы |
| `adapters/memory/in_memory_entity_registry.py` (`InMemoryEntityRegistry`) | `EntityRegistry` | L1 (алиас точное совпадение, без учёта регистра) + L2 (чистый Python, cosine ≥ 0.85) + L3 (создание нового); потокобезопасный через Lock; тестовые хелперы `clear()`/`count()` |
| `adapters/memory/postgres_entity_registry.py` (`PostgresEntityRegistry`) | `EntityRegistry` | Тот же L1/L2/L3 поверх `agent_N.entities` + `agent_N.entity_aliases`; `halfvec` cosine для L2; защищённый импорт psycopg3 |
| `adapters/memory/in_memory_entity_stance.py` (`InMemoryEntityStanceStore`) | `EntityStanceStore` | цепочка supersession; потокобезопасный |
| `adapters/memory/postgres_entity_stance.py` (`PostgresEntityStanceStore`) | `EntityStanceStore` | цепочка supersession в `agent_N.entity_stance`; разрешение serial_id по агенту; psycopg3 |
| `adapters/memory/in_memory_memory_guardian.py` (`InMemoryMemoryGuardian`) | `MemoryGuardian` | scan_orphan_entities + scan_merge_candidates + scan_stale_moments + scan_embedding_gaps + жизненный цикл findings (write/get_unresolved/resolve); **HLE-31** также реализует `scan_quality_metrics` — проверки на скользящем окне для `affect_detector_silent` (частота incomplete_coloring), `divergence_pattern` (один и тот же divergence_type ≥ N раз в окне через опциональный `DivergenceEventStore`), `stance_formation_too_fast` (stance сформирован < 24ч после опорных moments через опциональный `EntityStanceStore`). Отсутствующие опциональные входы ⇒ под-скан молча выдаёт ноль findings. **HLE-32** также реализует встроенные post-write проверки: факт без `metadata['embedding']` → `embedding_missing`; сущность без эмбеддинга (не value/principle) → `embedding_missing`; KeyMoment с `incomplete_coloring=True` → `affect_detector_silent` (severity `info`). Все встроенные проверки дедуплицируются относительно нерешённых findings для одного и того же `(target_table, target_id, finding_type)` |
| `adapters/memory/noop_reranker.py` (`NoOpReranker`) | `MemoryReranker` | passthrough — возвращает кандидатов, отсортированных по существующему score; для развёртывания без модели reranker'а |
| `adapters/memory/bge_reranker.py` (`BgeReranker`) | `MemoryReranker` | `BAAI/bge-reranker-v2-m3` через FlagEmbedding; ленивая загрузка; защищённые импорты; прозрачный fallback на исходный порядок score при сбое инференса |
| `adapters/linguistic/noop_adapter.py` (`NoOpLinguisticAnalyzer`) | `LinguisticAnalyzer` | возвращает пустые, но валидные объекты анализа; используется по умолчанию при `LINGUISTIC_ENABLED=false` |
| `adapters/linguistic/gliner_minilm_adapter.py` (`GLiNERPlusMiniLMAdapter`) | `LinguisticAnalyzer` | GLiNER (`urchade/gliner_multi-v2.1`) + MiniLM NLI (`MoritzLaurer/multilingual-MiniLMv2-L6-mnli-xnli`); ленивая загрузка модели; защищённые импорты; эвристики дивергенции для русского языка; требует `pip install -e ".[linguistic]"`; **opt-2**: кеш на основе SHA-256 в рамках сессии на `analyze_user_message()` — повторные упоминания пропускают инференс GLiNER+MiniLM; `clear_session_cache()` вызывается runner'ом при завершении сессии |
| `adapters/maintenance/in_memory_queue.py` (`InMemoryMaintenanceQueue`) | `MaintenanceQueue` | идемпотентность по run_key; атомарный `claim_batch`; все переходы статусов |
| `adapters/maintenance/postgres_queue.py` (`PostgresMaintenanceQueue`) | `MaintenanceQueue` | `claim_batch` через SKIP LOCKED CTE над `public.maintenance_jobs`; идемпотентность по run_key; psycopg3 |
| `adapters/linguistic/mrebel_adapter.py` (`MRebelRelationAdapter`) | `EntityRelationExtractor` | `Babelscape/mrebel-large` через transformers `text2text-generation`; ленивая загрузка; парсер REBEL-триплетов; защищённые импорты |
| `adapters/reflection/state_store_session_repository.py` (`StateStoreSessionRepository`) | `SessionRepository` | тонкий адаптер поверх любого `StateStore` (InMemory / File / Postgres v2); слот конструктора с `agent_id` по умолчанию для однaгентных развёртываний + явная трёхаргументная форма для мультиагентных реестров; нормализует границы диапазона и устаревшие naive-таймстампы сессий/moments в UTC и предупреждает, когда клиентский лимит выборки насыщается; основа для R3+R4 (миграция Daily/Deep-рефлексии) |
| `adapters/storage/in_memory_reflection_store.py` | `PatternStore`, `ReflectionEventStore`, `HealthAssessmentStore` | хранилища вывода рефлексии |
| `adapters/storage/in_memory_self_applied_changes.py` (R11.5) | `SelfAppliedChangeStore` | append-only аудит; поддерживает откат путём возврата к снапшоту до изменения |
| `adapters/storage/postgres_self_applied_changes.py` (R11.5) | `SelfAppliedChangeStore` | `agent_{N}.self_applied_changes`; привязан к одному `agent_id` при конструировании |
| `adapters/storage/in_memory_pending_human_review.py` (R11.7) | `PendingHumanReviewInbox` | упорядочивание priority-first / oldest-first; разрешение устанавливает resolved_at + applied_change_id |
| `adapters/storage/postgres_pending_human_review.py` (R11.7) | `PendingHumanReviewInbox` | `agent_{N}.pending_human_review`; помещает `agent_id` в `context` |
| `adapters/storage/reflection_store.py` (`ReflectionStore`) | — | PostgreSQL `agent_{N}.reflections` через `AgentSchemaResolver` (без RLS); **перенесён сюда из `atman/reflection/store.py` в HLE-53 — старый пакет упразднён, модель персистенса находится в `adapters/storage/reflection_persistence_models.py` и повторно использует канонический `core.models.reflection.ReflectionLevel`** |
| `adapters/storage/_atomic_write.py` (`write_atomically`) (HLE-49) | — | Общий хелпер tempfile + fsync + chmod + os.replace; лежит в основе `FileBackend._save_facts` и `FileStateStore._write_json_atomically`, так что контракт атомарной записи задан в одном месте |
| `adapters/clock.py` (`FrozenClock`) (HLE-54) | `ClockPort` | Замороженные часы для тестовых fixture; дефолтные `SystemClock` находятся в `core/clock_impl.py`, так как это zero-dep обёртка над stdlib |
| `adapters/storage/postgres_agent_schema.py` | — | разрешение `agent_id` → схема `agent_{serial_id}` для субъектных Postgres-адаптеров |
| `adapters/storage/in_memory_reflection_request_queue.py` (R12) | `ReflectionRequestQueue` | идемпотентность в рамках UTC-часового bucket через `agent_driven_run_key(reason, hour)` |
| `adapters/observability/in_memory_overload_alert_sink.py` (R13) | `ReflectionOverloadAlertSink` | захватывает алерты в памяти; сбои sink'а подавляются, чтобы монитор не мог обрушить вызывающий код |
| `adapters/observability/logging_overload_alert_sink.py` (HLE-30) | `ReflectionOverloadAlertSink` | направляет алерты через стандартный модуль `logging` на уровне CRITICAL/WARNING; структурированный payload (severity + details) прикрепляется через `extra` для JSON-форматтеров; сбои логирования подавляются |
| `adapters/observability/composite_overload_alert_sink.py` (HLE-30) | `ReflectionOverloadAlertSink` | fan-out обёртка над упорядоченным кортежем sink'ов; исключения по каждому sink'у подавляются, чтобы один нестабильный downstream не блокировал остальные |
| `adapters/observability/sentry.py` | — | legacy Sentry-хелперы (`init_sentry_from_env`, scope tags, spans, metrics, `cron_checkin`); no-op, если ни этот адаптер, ни `observability.init_observability()` не инициализировали SDK; предпочтительно использовать `init_observability()` в точках входа CLI — не вызывать `init_sentry_from_env()` после него (повторная инициализация перезаписывает sampling/scrubbing) |
| `observability/sentry_init.py` (HLE-240) | — | канонический `init_observability(level)` / `is_enabled()`; уровни `ATMAN_OBS_LEVEL` (`off` \| `minimal` \| `debug` \| `verbose`); `off` никогда не импортирует `sentry_sdk`; уровни настраивают `traces_sampler`, scrubbing, Spotlight, профилирование согласно `docs/features/observability/levels.md` |
| `observability/sampling.py` | — | `_traces_sampler`: AI-роуты / `gen_ai.*` с коэффициентом 1.0, по умолчанию 0.1 |
| `observability/scrubbing.py` | — | расширенный denylist + `before_send` / `before_send_transaction` (отбрасывает `/health`, `/healthz`) |
| `observability/spans.py` | — | `ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`, `pipeline_span`, `job_scope` — общие имена операций для сканера инструментирования |
| `core/session_log.py` | — | JSONL debug-лог сессии (`ATMAN_SESSION_LOG`); `slog()` + опциональные `set_display_hook` / `get_display_hook` для live UI и цепочки observability |
| `adapters/agent/pending_reviews_context.py` (R11.7) | — | хелпер `format_pending_reviews_block`: priority-first, oldest-first, усечение контекста |
| **`adapters/state/postgres_state_store.py`** (`PostgresStateStore`) | **`StateStore`** | **PostgreSQL v2** — схемы по агентам (`agent_N.sessions`, `agent_N.key_moments`); полный Session API (`create_session`, `get_session`, `update_session`, `list_recent_sessions`) и v2 KeyMoment API (`create_key_moment`, `store_key_moment` upsert, `mark_moment_accessed`, `update_moment_structured_markers`, `find_moments_by_entity`); разрешение схемы через фиксированный `serial_id` или кешированный поиск по `public.agents`; Identity/Narrative/Eigenstate всё ещё `NotImplementedError` (обслуживаются `FileStateStore`) |
| **`adapters/storage/in_memory_postgres_reflection_store.py`** (`InMemoryReflectionStore`) | **`ReflectionStore`** | **E27**: in-memory с симуляцией BIGSERIAL + RLS |
| `adapters/storage/reflection_persistence_helper.py` | — | **E27**: вспомогательные функции для персистенса рефлексий (`persist_micro_reflection`, `persist_daily_reflection`, `persist_deep_reflection`) |
| `adapters/reflection/mock_reflection_model.py` (`MockReflectionModel`) | `ReflectionModel` | детерминированная заглушка |
| **`adapters/reflection/openai_reflection_model.py`** (**`OpenAIReflectionModel`**) | **`ReflectionModel`** | **Универсальный OpenAI-совместимый адаптер** с `OpenAILLMConfig` (base_url, api_key, model, timeout, настраиваемые повторы); **`adapters/reflection/__init__.py`** экспортирует фабрику **`get_reflection_model()`** (env `ATMAN_REFLECTION_BACKEND=openai|anthropic|mock`, по умолчанию: `openai`) |
| `adapters/reflection/fixture_loader.py` | — | загрузка fixture'ов для демо |
| `adapters/agent/config.py` (`ModelConfig`, `AgentConfig`) | — | Конфигурация модели Pydantic AI + рантайм-конфигурация агента: лимиты контекстного окна, таймаут сессии, переключатель free-time, видимость monologue, **режим внедрения memory** (`assistant_message`/`user_message`/`system_prompt` для универсальной доставки memory-контекста) (E22.1, E26-R1, E26-R2, E26-R4); **opt-2**: `rag_token_budget` (по умолчанию 2000 токенов), `enable_prompt_caching` (разделение stable/dynamic контекста), `max_moments_per_reflection` (по умолчанию 30) |
| `adapters/agent/deps.py` (`AtmanDeps`, `AtmanDeps.from_config`) | — | замороженный DI-контейнер, связывающий `SessionManager`, `IdentityService`, `ExperienceService`, `MicroReflectionService`, `StateStore`; фабрика `from_config` переносит валидированные лимиты из `AgentConfig`; опциональное поле `injected_context` для режима внедрения `system_prompt`; **R11.7/R12** опциональные поля `pending_review_inbox` и `reflection_request_queue`, управляющие регистрацией инструментов; **opt-2** опциональное поле `passive_memory_injector`; **WP-08 v2** опциональное поле `skill_manager: SkillManagerPort | None = None`; **HLE-29** опциональное поле `divergence_event_store: DivergenceEventStore | None = None`, предоставляющее персистентный поток дивергенции, чтобы R6 DivergenceAggregator (и произвольные читатели) могли его потреблять, не залезая во внутренности `SessionManager`; **HLE-30** опциональные `reflection_overload_monitor: ReflectionOverloadMonitor | None = None` + `overload_alert_inspect: InMemoryOverloadAlertSink | None = None` (in-memory отвод внутри composite sink, вынесенный наружу для admin UI / debug-чтения без обращения к приватной цепочке `_sink._sinks` монитора); **HLE-32** опциональное поле `memory_guardian: MemoryGuardian | None = None` — тот же экземпляр guardian'а, что подан во встроенный pipeline, вынесенный наружу, чтобы cli_maintenance / admin-эндпоинты читали findings (`get_unresolved`) без пересборки цепочки; **HLE-33** опциональное поле `ambient_memory: AmbientMemoryService | None = None` — параллельный RAG на основе entity-anchor, вызывающие обращаются к `compose_injection(text, agent_id=...)` напрямую через deps |
| `adapters/agent/memory_injection.py` (`inject_memory`, `MemoryInjectionMode`) | — | Универсальное внедрение memory с тремя режимами: (1) `assistant_message` — вставляет `ModelResponse` в начало истории (по умолчанию; совместимо с OpenAI/Ollama), (2) `user_message` — оборачивает memory как ход пользователя (совместимо с Anthropic), (3) `system_prompt` — устанавливает `deps.injected_context` для добавления через `build_instructions` (legacy путь system-prompt в pydantic-ai) |
| `adapters/agent/instructions.py` (`build_instructions`, `build_memory_context`, `build_skill_suggestions_section`) | — | `build_instructions`: строит поведенческие правила (как агент использует инструменты, обязательства); Идентичность/Нарратив перенесены в `build_memory_context()` для доставки через `inject_memory()`; при `memory_injection_mode == "system_prompt"` добавляет `deps.injected_context`; **WP-08 v2**: `_build_pinned_skills_section(deps)` внедряет список закреплённых навыков, когда `deps.skill_manager` не None; `build_skill_suggestions_section(suggestions)` рендерит `## Релевантные навыки` для попаданий per-turn `trigger_router` (runner добавляет через `inject_memory`); `_build_self_awareness_section(deps)` внедряет самоописание Atman (memory, навыки, рефлексия); ошибки перехватываются → fallback на пустую строку |
| `adapters/agent/tools.py` (`record_key_moment` async, `log_experience`, `restart_session`, `wait_session`, `resolve_pending_review`, `request_reflection`) | — | Инструменты Pydantic AI: `record_key_moment` → `AffectDetector.submit_self_report`, когда `SessionManager` сконфигурирован с affect; `log_experience` — заглушка-редирект; `restart_session` / `wait_session` возвращают строки-сентинелы для управления сессией (E22.4); **R11.7** `resolve_pending_review` → `PendingHumanReviewInbox.resolve` (регистрируется только когда inbox подключён); **R12** `request_reflection` → `ReflectionRequestQueue.enqueue` с идемпотентным ключом по часовому bucket (регистрируется только когда очередь подключена) |
| `adapters/agent/factory.py` (`build_deps`, `build_daily_reflection_service`, `build_deep_reflection_service`, `_build_skill_manager`, `_build_reflection_model`, `_StateStoreIdentityRepo`, `_StateStoreNarrativeRepo`) | — | Собирает `AtmanDeps`, `SessionManager`, `FileStateStore`, сервисы, опциональный `AffectDetector` из workspace + `AgentConfig`; бутстрапит недостающую Идентичность/Нарратив для новых runner-workspace'ов и связывает `SessionManager(workspace=...)`, чтобы crash-журналы были активны; **HLE-27**: собирает `InMemoryMaintenanceQueue` + `PostWriteScheduler` и передаёт scheduler в `SessionManager`, чтобы завершённые сессии рассылали задания mREBEL / lingvo-обогащения в очередь — очередь накапливается, пока отдельный процесс `python -m atman.cli_maintenance run --loop` не разберёт её (в продакшене: замена на `PostgresMaintenanceQueue` + отдельный worker pod); **HLE-29**: безусловно собирает `NoOpLinguisticAnalyzer` + `DivergenceDetector(agent_id)` + `InMemoryDivergenceEventStore` и подключает их в `AffectDetector`, созданный через `session_manager.attach_affect(...)`, чтобы pipeline дивергенции персистировал события из коробки (реальный GLiNER/mREBEL-анализатор подключается позже через extra `linguistic`); **opt-2**: условно инстанцирует `PassiveMemoryInjector` (embedding + factual_memory + state_store + zero-dep `BM25EmbeddingAdapter` для RRF-фьюжна + `NoOpLinguisticAnalyzer` всегда + fallback `BgeReranker` / `NoOpReranker`), когда `ATMAN_LINGUISTIC_ENABLED=true`; `NoOpLinguisticAnalyzer` подключается всегда, чтобы ambient_mode был достижим в лёгких развёртываниях; `BgeReranker` пробуется первым (требует FlagEmbedding + ассеты модели), с fallback на `NoOpReranker`; сбой всего блока не фатален (логирует предупреждение, RAG остаётся отключённым); **WP-08 v2**: собирает `SkillManager` через `_build_skill_manager()` ДО `MicroReflectionService`, чтобы хук рефлексии (`process_session_skills`) действительно срабатывал — передаёт его и в `MicroReflectionService(skill_manager=…)`, и в `AtmanDeps`; `_build_skill_manager` сначала пробует `PostgresSkillStore(db_url, agent_id=agent_id)`, откатывается на `InMemorySkillStore`, когда PostgreSQL недоступен (локальная разработка без PostgreSQL), и в конце возвращает `None`, если ни одно хранилище не удалось построить; **`build_daily_reflection_service` / `build_deep_reflection_service`**: связывают `DailyReflectionService` / `DeepReflectionService` поверх `StateStoreSessionRepository` + `_StateStoreIdentityRepo` (+ `_StateStoreNarrativeRepo` для deep) с хуками R5–R10 (stance/relations сущностей, triage findings, кандидаты на merge); `_build_reflection_model` выбирает `OpenAIReflectionModel`, когда установлен `ATMAN_LLM_BASE_URL`, иначе `_MockReflectionModel`; билдеры рефлексии вызывают `_optional_embedding_adapter()` перед `_build_skill_manager`, чтобы отсутствие FlagEmbedding не прерывало конструирование сервиса |
| `adapters/agent/preflight.py` (`run_cli_preflight`, `run_streamlit_preflight`, `check_nlp_packages`, `install_nlp`, `_streamlit_restart_argv`) | — | Preflight-гейт сессии: обязательные NLP-пакеты (опциональная установка + перезапуск процесса), доступность PostgreSQL, проверка LLM-эндпоинта; неблокирующий прогрев модели в фоне; вариант для Streamlit использует `st.stop()` при жёстких сбоях и `_streamlit_restart_argv()` для перезапуска `python -m streamlit run …` после установки NLP |
| `adapters/observability/sentry.py` (`init_sentry_from_env`, `capture_silent_exception`, `metric_distribution`, `metric_gauge`, `metric_increment`) | — | Legacy-хелперы Sentry SDK; `init_sentry_from_env` теперь делегирует в `atman.observability.init_observability`, так что оба пути инициализации сходятся воедино; проставляет теги agent/session в scope; хелперы метрик с API `attributes=` и legacy fallback `tags`/`incr`; все вызовы no-op при отсутствии SDK |
| `observability/__init__.py` (`init_observability`, `is_enabled`, `ObsLevel`) | — | Новая унифицированная точка входа observability (HLE-239 P1.1); читает env-переменную `ATMAN_OBS_LEVEL` (off/minimal/debug/verbose); настоящий no-op при "off" (без импорта sentry_sdk); `minimal` включает sampling + PII-scrubbing; `debug` добавляет Spotlight + профили; `verbose` добавляет stack traces и локальные переменные |
| `observability/sentry_init.py` | — | Логика инициализации Sentry SDK; настраивает `AsyncioIntegration`, `HttpxIntegration`, `LoggingIntegration`; `AnthropicIntegration` подключается условно, когда доступен пакет `anthropic` (авто-трейсинг LLM); PII-scrubbing через `EventScrubber`; sampling AI-операций повышен до 1.0 через `_traces_sampler`; транзакции health-роутов отбрасываются через `before_send_transaction` |
| `observability/spans.py` (`ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`, `pipeline_span`, `job_scope`) | — | Слой хелперов span для инструментирования критического пути; все хелперы делают lazy-import sentry_sdk, поэтому корректно превращаются в no-op при отключённом Sentry; `job_scope` оборачивает фоновые задания в isolation_scope для независимой группировки ошибок |
| `adapters/agent/runner.py` (`AtmanTurn`, `AtmanRunner`, `chat`, `_force_finish`, `_check_restart_requested`, `_do_restart`, `_build_restart_package`, `_check_token_usage`, `_start_stdin_reader`, `_stop_stdin_reader`, `_handle_menu_mode`, `_handle_free_time_mode`, `_call_trigger_router_if_enabled`) | — | **`AtmanTurn`**: pre/post pipeline на каждый ход для хост-агентов (Streamlit `3_Chat.py`, `session_tester.py`) — очищает `injected_context` на каждом ходу, passive/ambient RAG, поведенческие подсказки навыков + предложения `trigger_router` через `_inject_skill_suggestions`, анализ ответа, авто key moments, `record_event` → AffectDetector, захват отказов по ценностям, слив maintenance; сигнал-осведомлённая обёртка жизненного цикла сессии с циклом рестарта, мониторингом токенов и таймаутом/меню (E22.2, E22.3, E22.5, E22.6); мониторинг токенов: прогрессивные предупреждения на 70/80/90%, принудительное закрытие на 95% (`_check_token_usage`); stdin reader на основе очереди (без гонки при таймауте); обнаружение рестарта: сентинел → завершение сессии с `close_reason="restart"` → сборка пакета (key moments + причина + хвост) → новая сессия с обновлённым `AtmanDeps`; таймаут сессии → режим меню (reflect/wait/sleep/save_to_memory/free_time); SIGTERM/KeyboardInterrupt/EOFError/SystemExit → graceful `_force_finish()`; создаёт минимальный `KeyMoment`, если пусто; сохраняет коды выхода; **R11.7** в начале сессии выводит топ нерешённых элементов `PendingHumanReviewInbox` первым системным сообщением и условно регистрирует инструменты `resolve_pending_review` / `request_reflection`, когда их зависимости присутствуют в `AtmanDeps`; **opt-2**: инициализирует `SessionWorkingMemory` + `SessionCache` на сессию; вызывает `surface_for_context()` + `build_rag_context(budget=rag_token_budget)` перед каждым `agent.run()`; очищает все кеши сессии в блоке `finally`; вызывает `la.clear_session_cache()` на `LinguisticAnalyzer`, если он присутствует; **WP-08 v2 / PR #604**: перед каждым ходом вызывает `skill_manager.collect_behavioral_hints_from_message(user_text, …)` (ошибки подавляются), затем `_call_trigger_router_if_enabled` → `build_skill_suggestions_section` → добавление через `inject_memory` (эфемерно, как RAG; удаляет предыдущее сообщение с историей навыков даже если текущий ход не даёт предложений) |
| `agents_registry.py` (`AgentsRegistry`) | — | PostgreSQL-хранимый реестр экземпляров агентов (URL app/admin БД); используется `src/run_agent.py` |
<!-- codemap:auto:end -->
...
| `skills/port.py` | **Shim устаревания** — re-export `SkillManagerPort` из `atman.core.ports.skill_manager` (`DeprecationWarning`); канонический импорт: `from atman.core.ports.skill_manager import SkillManagerPort` | — |
...
| `skills/store.py` | `SkillStore` Protocol — интерфейс хранилища | … `mark_processed`, `list_invocations_by_skill` (PR #604: CLI inspect), `count_invocations_in_last_n_sessions` (PR #604: скользящее окно auto-pin) |
...
| `skills/manager.py` | Реализация `SkillManager` | полный lifecycle; capture/install_external + `EntityRegistry.resolve_or_create(EntityType.skill)` (fallback `uuid4()`); auto-pin через `count_invocations_in_last_n_sessions`; `process_daily_skills` промоутит draft→active; `collect_behavioral_hints_from_message` |
...
| `skills/cli.py` | CLI навыков (`atman-skills`), Rich через `atman.term` | … `inspect-invocations` (PR #604: таблица из `list_invocations_by_skill`), `capture-manual` (PR #604), `install-external`, `revise`; read-only при `enabled=false` |
...
**Конфиг (HLE-36)**: `daily_revision_idle_bump_sessions` (5), `deep_archive_sessions` (50), `deep_failure_rate_threshold` (0.5), `deep_min_invocations_for_failure_rate` (5), `draft_promote_min_successes` (3, PR #604).
...
| `web_dashboard/pages/3_Chat.py` | web | живой чат агента (gemma4): `AtmanTurn` pre/post, стриминг токенов, debug-панель (Events / KM / Facts / RAG), завершение сессии; `run_streamlit_preflight()` при старте |
| `web_dashboard/pages/4_Status.py` | web | статус компонентов (Postgres, LLM, NLP warmup); опциональная установка NLP + restart |
| `web_dashboard/utils/chat_deps.py` (`get_chat_deps`, `install_slog_hook`, `_workspace_path`) | web | фабрика deps для чата: `ATMAN_CURRENT_AGENT` / `agent_id.txt`, единый порядок workspace с live runners |
| `web_dashboard/utils/cmd.py`, `web_dashboard/utils/runner.py` | web | подпроцессы |
| `scripts/show_agent_data.py` | CLI | Rich-таблицы: KM, факты, сущности (`psycopg.sql.Identifier` для имён схем) |
...
| `e2e/live_chat.py` | e2e | интерактивный REPL: gemma4 ↔ live `AtmanTurn`; `run_cli_preflight()`; инструменты record_key_moment, restart, wait, resolve_pending_review, request_reflection |
| `e2e/session_tester.py` | e2e | автотестер сценариев (`make session-test`): динамические follow-up, `AtmanTurn.pre/post`, отчёт метрик |
...
| PR #604 | Пробелы skill-manager-design: `trigger_router` не был в runner; draft не промоутился; behavioral hints только в конце сессии; `inspect-invocations` — SQL-заглушка; auto-pin по lifetime count; `SkillManagerPort` жил в `atman.skills` | покрыто (`tests/test_skill_trigger_in_runner.py`, `tests/test_skill_daily_deep_hooks.py::TestDraftPromotion`, тесты store/manager для новых методов) |
