...
| `adapters/observability/sentry.py` | — | opt-in инициализация Sentry SDK через `SENTRY_DSN`; scope-теги (`agent_id`, `session_id`); `session_transaction` / `pipeline_span` / spans maintenance/reflection (один yield, no-op при отключении); capture тихих исключений; helpers метрик; `install_slog_breadcrumb_hook()` прокидывает каждый `slog()` в Sentry breadcrumbs, сохраняя уже зарегистрированный display hook (например Rich-консоль live_chat) |
