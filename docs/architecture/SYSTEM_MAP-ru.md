...
| текущий PR | Генератор синтетического RU NER-датасета считал менее 1500 примеров лишь предупреждением и затем перезаписывал существующий датасет, в том числе при нуле пригодных строк от API | покрыто (`tests/scripts/test_generate_synthetic_ru.py::test_main_preserves_existing_dataset_when_generation_is_incomplete`) |
