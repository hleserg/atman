...
| `observability/sentry_init.py` (HLE-240) | — | канонический `init_observability(level)` / `is_enabled()` / `is_full_mode()`; уровни `ATMAN_OBS_LEVEL` (`off` \| `minimal` \| `debug` \| `verbose` \| `full`); `full` включает `send_default_pii=True`, все промпты, без фильтрации, стриминг AI-спанов (НЕ для продакшена); `off` не импортирует `sentry_sdk`; уровни подключают `traces_sampler`, scrubbing, Spotlight, profiling — см. `docs/features/observability/levels.md` |
...
| `observability/__init__.py` (`init_observability`, `is_enabled`, `is_full_mode`, `ObsLevel`) | — | единая точка входа observability (HLE-239 P1.1); читает `ATMAN_OBS_LEVEL` (off/minimal/debug/verbose/full); true no-op на "off" (sentry_sdk не импортируется); `minimal` — семплирование + PII-скрабинг; `debug` — Spotlight + профилирование; `verbose` — стектрейсы и локальные переменные; `full` — все PII/промпты/NLP-данные, НЕ для продакшена |
