...
| текущий PR | Подстановка пароля в PostgreSQL URL не экранировала зарезервированные символы, игнорировала libpq `PGPASSWORD`, а factory проверял DSN, отличный от DSN persistence-адаптера | исправлено: разрешение пароля сохраняет host/path DSN, а factory повторно использует единый разрешённый URL; покрыто `tests/test_db_url.py` и `tests/test_agent_reflection_factory_builders.py::test_build_state_store_uses_same_resolved_url_for_probe_and_adapter` |
