...
| `observability/scrubbing.py` | — | расширенный denylist + `before_send` / `before_send_transaction` (отбрасывает `/health`, `/healthz`); `ATMAN_SEND_PROMPTS=1` может убрать raw LLM I/O ключи только при явном local/dev `SENTRY_ENVIRONMENT`, а production/CI/default окружения продолжают скрабить prompts |
...
| `observability/sentry_init.py` | — | логика инициализации Sentry SDK; `AsyncioIntegration`, `HttpxIntegration`, `LoggingIntegration`; `AnthropicIntegration` подключается условно при наличии пакета `anthropic` (автоматическая трассировка LLM); PII-скрабинг через `EventScrubber` с local/dev-only passthrough для raw prompts; семплирование AI-операций 1.0 через `_traces_sampler`; health-route транзакции отбрасываются через `before_send_transaction` |
