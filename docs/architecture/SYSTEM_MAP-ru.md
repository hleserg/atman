...
| `adapters/observability/sentry.py` | — | legacy-хелперы Sentry (`init_sentry_from_env`, scope-теги, spans, метрики, `cron_checkin`); `install_slog_breadcrumb_hook` зеркалирует session-log события в Sentry через `_sentry_slog_attrs`, который отбрасывает raw memory/user text fields перед внешними breadcrumbs/log attributes; no-op, если SDK не инициализирован ни адаптером, ни `observability.init_observability()`; в CLI предпочтителен `init_observability()` — не вызывать `init_sentry_from_env()` после него (повторный init перезаписывает sampling/scrubbing) |
...
| `adapters/observability/sentry.py` (`init_sentry_from_env`, `install_slog_breadcrumb_hook`, `_sentry_slog_attrs`, `capture_silent_exception`, `metric_distribution`, `metric_gauge`, `metric_increment`) | — | устаревший слой Sentry-хелперов; `init_sentry_from_env` делегирует `atman.observability.init_observability`, чтобы оба пути инициализации сходились; теги agent/session; атрибуты slog breadcrumbs/logs редактируются на границе адаптера до вызовов Sentry SDK; метрики с API `attributes=` и fallback на legacy `tags`/`incr`; no-op без SDK |
...
| `src/atman/eval/gliner2/dataset.py`, `src/atman/eval/gliner2/baseline.py` | offline eval | HLE-379 zero-shot baseline GLiNER2 для русского NER: типизированные `DatasetExample` / `EntitySpan` с character-span gold set, загрузчик GLiNER2/legacy GLiNER, strict-метрики через nervaluate и merge JSON-результатов в `eval/results/gliner2_baseline_ru.json`; opt-in запуск: `python -m atman.eval.gliner2.baseline --model <hf-model-id>` |
...
| текущий PR | Sentry `slog` breadcrumb hook зеркалировал raw memory/user text поля (`content`, `what_happened`, `query`, `anchor`, `anchors`, `text`) во внешние breadcrumb/log attributes, несмотря на event-level scrubbing | покрыто (`tests/test_sentry_slog_hook.py::test_sentry_slog_attrs_redacts_memory_and_user_text`, `tests/test_sentry_slog_hook.py::test_sentry_slog_hook_keeps_sensitive_data_out_of_breadcrumb`) |
