...
| `adapters/agent/runner.py` (`AtmanRunner`, `chat`, …, `_call_trigger_router_if_enabled`) | — | обёртка жизненного цикла сессии (E22.2–E22.7), restart loop, мониторинг токенов, menu mode; **R11.7** pending-review inbox; **opt-2**: RAG per-turn; **WP-08 v2 / PR #604**: перед каждым ходом `collect_behavioral_hints_from_message(user_text, …)` (ошибки подавляются), затем `_call_trigger_router_if_enabled` → `build_skill_suggestions_section` → `inject_memory` (эфемерно, как RAG) |
