...
| `observability/sentry_init.py` | — | логика инициализации Sentry SDK; `AsyncioIntegration`, `HttpxIntegration`, `LoggingIntegration`; PII-скрабинг через `EventScrubber`; семплирование AI-операций 1.0 через `_traces_sampler`; health-route транзакции отбрасываются через `before_send_transaction` |
| `observability/spans.py` (`ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`) | — | span-хелперы для critical-path инструментирования; ленивый импорт sentry_sdk, no-op при отключённом Sentry |
