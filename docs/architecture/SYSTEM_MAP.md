...
| `adapters/observability/sentry.py` (`init_sentry_from_env`, `capture_silent_exception`, `metric_distribution`, `metric_gauge`, `metric_increment`) | — | Optional Sentry SDK wiring from env (`SENTRY_DSN`); scopes agent/session tags; metrics helpers with `attributes=` API and legacy `tags`/`incr` fallback; all calls no-op when SDK missing |
