...
| `adapters/storage/file_state_store.py` (`FileStateStore`) | `StateStore` | JSON files (experience + identity + narrative + eigenstate) + `key_moments.jsonl`; **v2**: `list_key_moments(session_id)` filtering support; `update_moment_structured_markers` persists lingvo markers to per-moment files, JSONL, and any matching per-session bundle |
