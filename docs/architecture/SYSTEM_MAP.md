...
| `adapters/observability/sentry.py` | — | opt-in Sentry SDK init via `SENTRY_DSN` (`enable_logs=True`, requires `sentry-sdk>=2.35.0`); scope tags (`agent_id`, `session_id`); `session_transaction` / `pipeline_span` / maintenance/reflection spans (single-yield context managers, no-op when disabled); silent-exception capture; metrics helpers (`attributes=` with legacy `tags=` / `incr` fallback); `install_slog_breadcrumb_hook()` emits `sentry_sdk.logger` records plus breadcrumbs for every `slog()` while preserving any existing display hook (e.g. live_chat Rich console) |
