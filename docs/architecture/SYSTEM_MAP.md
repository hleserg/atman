...
| `observability/scrubbing.py` | — | extended denylist + `before_send` / `before_send_transaction` (drops `/health`, `/healthz`); `ATMAN_SEND_PROMPTS=1` can remove raw LLM I/O keys only in explicit local/dev `SENTRY_ENVIRONMENT` values, while production/CI/default environments keep prompts scrubbed |
...
| `observability/sentry_init.py` | — | Sentry SDK initialisation logic; configures `AsyncioIntegration`, `HttpxIntegration`, `LoggingIntegration`; `AnthropicIntegration` conditionally loaded when `anthropic` package is available (auto LLM tracing); PII scrubbing via `EventScrubber` with local/dev-only raw prompt passthrough; AI operation sampling boosted to 1.0 via `_traces_sampler`; health-route transactions dropped via `before_send_transaction` |
