...
| `observability/spans.py` | — | `ai_chat_span`, `memory_span`, `db_span`, `cron_span` — shared op names for instrumentation scanner |
...
| `observability/sentry_init.py` | — | Sentry SDK initialisation logic; configures `AsyncioIntegration`, `HttpxIntegration`, `LoggingIntegration`; PII scrubbing via `EventScrubber`; AI operation sampling boosted to 1.0 via `_traces_sampler`; health-route transactions dropped via `before_send_transaction` |
| `observability/spans.py` (`ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`) | — | Span helper layer for critical-path instrumentation; all helpers lazy-import sentry_sdk so they no-op gracefully when Sentry is disabled |
...
| `core\models\narrative.py` | `Eigenstate`, `NarrativeThread`, `LayerType`, `NarrativeLayer`, `NarrativeDocument` |  |
...
| `core\services\narrative_revision.py` | `NarrativeRevisionService` |  |
| `core\services\narrative_service.py` | `NarrativeService` |  |
...
| `adapters\agent\token_monitor.py` | `TokenMonitor`, `ContextLimitExceeded` |  |
...
| `adapters\linguistic\noop_adapter.py` | `NoOpLinguisticAnalyzer` |  |
