...
| `adapters/observability/sentry.py` | — | opt-in Sentry SDK init via `SENTRY_DSN`; scope tags (`agent_id`, `session_id`); `session_transaction` / `pipeline_span` / maintenance/reflection spans (single-yield context managers, no-op when disabled); silent-exception capture; metrics helpers; `install_slog_breadcrumb_hook()` chains every `slog()` event into Sentry breadcrumbs while preserving any existing display hook (e.g. live_chat Rich console) |
