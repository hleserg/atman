...
| current PR | Synthetic RU NER generation treated fewer than 1,500 examples as a warning and then overwrote the existing dataset, including when the API returned zero usable rows | covered (`tests/scripts/test_generate_synthetic_ru.py::test_main_preserves_existing_dataset_when_generation_is_incomplete`) |
