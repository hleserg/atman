...
| `adapters/observability/sentry.py` | — | legacy Sentry helpers (`init_sentry_from_env`, scope tags, spans, metrics, `cron_checkin`); no-op when neither this adapter nor `observability.init_observability()` initialized the SDK; prefer `init_observability()` at CLI entry points — do not call `init_sentry_from_env()` after it (re-init overwrites sampling/scrubbing) |
...
| `adapters/observability/sentry.py` (`init_sentry_from_env`, `capture_silent_exception`, `metric_distribution`, `metric_gauge`, `metric_increment`) | — | Legacy Sentry SDK helpers; `init_sentry_from_env` now delegates to `atman.observability.init_observability` so both init paths converge; scopes agent/session tags; metrics helpers with `attributes=` API and legacy `tags`/`incr` fallback; all calls no-op when SDK missing |
...
| current PR | Password injection into PostgreSQL URLs did not percent-encode reserved characters, ignored libpq `PGPASSWORD`, and let the factory probe a different DSN from the persistence adapter | fixed: password resolution now preserves the DSN host/path and the factory reuses one resolved URL; covered by `tests/test_db_url.py` and `tests/test_agent_reflection_factory_builders.py::test_build_state_store_uses_same_resolved_url_for_probe_and_adapter` |
