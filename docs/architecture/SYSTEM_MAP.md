...
| `ecd0eee` | Synthetic Russian NER generation could overwrite `eval/data/atman_ner_ru_synth.jsonl` with a partial or empty replacement after malformed downloads or too-few converted rows | fixed: downloaded JSONL parsing is strict, the 1500-row minimum is fatal, and replacements are atomically written only after validation; covered by `tests/test_generate_synthetic_ru.py` |
