...
| `core/ports/maintenance_queue.py` | DB-backed cron queue for background maintenance jobs; SKIP LOCKED semantics, run_key idempotency; `validate_enqueue_payload` rejects moment-scoped jobs (`mrebel_extract`, `lingvo_enrich`) without `key_moment_id` in payload at enqueue time | `MaintenanceQueue` (ABC): `enqueue`, `claim_batch`, `mark_done`, `mark_failed`, `mark_skipped`, `list_jobs`; `validate_enqueue_payload` |
...
| `core/ports/skill_manager.py` (WP-08 v2) | **Canonical** skill-loop port consumed by all code outside `atman.skills`; `SkillManagerPort` is satisfied by both real `SkillManager` and `NoopSkillManager` | `SkillManagerPort` (runtime_checkable Protocol): `list_pinned`, `list_available`, `trigger_router`, `invoke`, `mark_result`, `capture`, `get_skill`, `process_session_skills`, `write_session_skills_marker` (HLE-35), `process_daily_skills` (HLE-36, promotes draft→active when `success_count ≥ draft_promote_min_successes`), `process_deep_skills` (HLE-36), `collect_behavioral_hints_from_message` (per-turn user-message scan) |
...
| `adapters/observability/sentry.py` | — | legacy Sentry helpers (`init_sentry_from_env`, scope tags, spans, metrics, `cron_checkin`); `install_slog_breadcrumb_hook` mirrors session-log events to Sentry through `_sentry_slog_attrs`, which drops raw memory/user text fields before external breadcrumbs/log attributes; no-op when neither this adapter nor `observability.init_observability()` initialized the SDK; prefer `init_observability()` at CLI entry points — do not call `init_sentry_from_env()` after it (re-init overwrites sampling/scrubbing) |
| `observability/sentry_init.py` (HLE-240) | — | canonical `init_observability(level)` / `is_enabled()`; `ATMAN_OBS_LEVEL` tiers (`off` \| `minimal` \| `debug` \| `verbose`); `off` never imports `sentry_sdk`; levels wire `traces_sampler`, scrubbing, Spotlight, profiling per `docs/features/observability/levels.md` |
| `observability/sampling.py` | — | `_traces_sampler`: AI routes / `gen_ai.*` at 1.0, default 0.1 |
| `observability/scrubbing.py` | — | extended denylist + `before_send` / `before_send_transaction` (drops `/health`, `/healthz`) |
| `observability/spans.py` | — | `ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`, `pipeline_span`, `job_scope` — shared op names for instrumentation scanner |
| `core/session_log.py` | — | JSONL session debug log (`ATMAN_SESSION_LOG`); `slog()` + optional `set_display_hook` / `get_display_hook` for live UIs and observability chaining |
...
| `adapters/agent/instructions.py` (`build_instructions`, `build_memory_context`, `build_skill_suggestions_section`) | — | `build_instructions`: builds behavioral rules (how agent uses tools, commitments); identity/narrative moved to `build_memory_context()` for delivery via `inject_memory()`; when `memory_injection_mode == "system_prompt"`, appends `deps.injected_context`; **WP-08 v2**: `_build_pinned_skills_section(deps)` injects pinned skills list when `deps.skill_manager` is not None; `build_skill_suggestions_section(suggestions)` renders `## Релевантные навыки` for per-turn `trigger_router` hits (runner appends via `inject_memory`); `_build_self_awareness_section(deps)` injects Atman self-description (memory, skills, reflection); errors are caught → empty string fallback |
...
| `adapters/observability/sentry.py` (`init_sentry_from_env`, `install_slog_breadcrumb_hook`, `_sentry_slog_attrs`, `capture_silent_exception`, `metric_distribution`, `metric_gauge`, `metric_increment`) | — | Legacy Sentry SDK helpers; `init_sentry_from_env` now delegates to `atman.observability.init_observability` so both init paths converge; scopes agent/session tags; slog breadcrumbs/log attributes are redacted at the adapter boundary before Sentry SDK calls; metrics helpers with `attributes=` API and legacy `tags`/`incr` fallback; all calls no-op when SDK missing |
| `observability/__init__.py` (`init_observability`, `is_enabled`, `ObsLevel`) | — | New unified observability entrypoint (HLE-239 P1.1); reads `ATMAN_OBS_LEVEL` env var (off/minimal/debug/verbose); true no-op on "off" (no sentry_sdk import); `minimal` enables sampling + PII scrubbing; `debug` adds Spotlight + profiles; `verbose` adds stack traces and local variables |
| `observability/sentry_init.py` | — | Sentry SDK initialisation logic; configures `AsyncioIntegration`, `HttpxIntegration`, `LoggingIntegration`; `AnthropicIntegration` conditionally loaded when `anthropic` package is available (auto LLM tracing); PII scrubbing via `EventScrubber`; AI operation sampling boosted to 1.0 via `_traces_sampler`; health-route transactions dropped via `before_send_transaction` |
| `observability/spans.py` (`ai_chat_span`, `ai_embeddings_span`, `ai_rerank_span`, `memory_span`, `db_span`, `cron_span`, `pipeline_span`, `job_scope`) | — | Span helper layer for critical-path instrumentation; all helpers lazy-import sentry_sdk so they no-op gracefully when Sentry is disabled; `job_scope` wraps background jobs in isolation_scope for independent error grouping |
...
| `skills/port.py` | **Deprecation shim** — re-exports `SkillManagerPort` from `atman.core.ports.skill_manager` (emits `DeprecationWarning`); canonical import is `from atman.core.ports.skill_manager import SkillManagerPort` | — |
...
| `skills/manager.py` | Real `SkillManager` implementation | `SkillManager`: full lifecycle — invoke (subprocess entry script, preliminary_status), mark_result, capture/install_external (creates SKILL.md + `EntityRegistry.resolve_or_create(EntityType.skill)` + row; `uuid4()` fallback when registry unavailable), `process_session_skills` (finalise invocations, update stats, auto-pin/downgrade via `count_invocations_in_last_n_sessions`, mark processed), `write_session_skills_marker` (HLE-35), `process_daily_skills` (HLE-36 + PR #604 draft→active when `success_count ≥ draft_promote_min_successes`), `process_deep_skills` (HLE-36), `collect_behavioral_hints_from_message` (positive/negative signals on open invocations) |
...
**Config (HLE-36)**: `SkillsSettings.daily_revision_idle_bump_sessions` (default 5), `deep_archive_sessions` (default 50), `deep_failure_rate_threshold` (default 0.5), `deep_min_invocations_for_failure_rate` (default 5), `draft_promote_min_successes` (default 3, PR #604).
...
| `cli_reflection.py` | CLI | Reflection Engine: `reflect micro|daily|deep` with `--fixtures` (mock) or `--live --agent-id <uuid> [--workspace .atman]` (real `FileStateStore`/`PostgresStateStore` + optional LLM via `ATMAN_LLM_BASE_URL`); deep live accepts `--since` / `--until` `YYYY-MM-DD` |
| `term.py` | utility | Rich output for CLI/demos |
| `observability/__init__.py` | observability | re-exports `init_observability`, `is_enabled`, `ObsLevel`; wired from `cli.py`, `cli_experience.py`, `cli_identity.py`, `cli_reflection.py`, `cli_maintenance.py` |
...
| `web_dashboard/pages/3_Chat.py` | web | Live agent chat (gemma4 via OpenAI-compatible API): `AtmanTurn` pre/post per message, streamed tokens, debug panel (Events / Key Moments / Facts / RAG), session finish + micro-reflection; `run_streamlit_preflight()` on load |
| `web_dashboard/pages/4_Status.py` | web | Component status dashboard (Postgres, LLM, NLP warmup); optional NLP install + `os.execv` restart |
| `web_dashboard/utils/chat_deps.py` (`get_chat_deps`, `install_slog_hook`, `_workspace_path`) | web | Chat UI deps factory: resolves `ATMAN_CURRENT_AGENT` / `agent_id.txt`, unified workspace path precedence with live runners |
| `web_dashboard/utils/cmd.py`, `web_dashboard/utils/runner.py` | web | subprocess helpers |
| `scripts/show_agent_data.py` | CLI | Rich tables: recent key moments, facts, entities for an agent schema (`psycopg.sql.Identifier` for schema names) |
...
| `e2e/live_chat.py` | e2e | Interactive REPL: gemma4 user-agent ↔ live `AtmanTurn` session; `run_cli_preflight()`; tools: `record_key_moment`, `restart_session`, `wait_session`, `resolve_pending_review`, `request_reflection` |
| `e2e/session_tester.py` | e2e | Automated multi-scenario live session tester (`make session-test`): dynamic follow-ups, `AtmanTurn.pre/post`, metrics report |
...
| `src/atman/eval/gliner2/dataset.py`, `src/atman/eval/gliner2/baseline.py` | offline eval | HLE-379 Russian GLiNER2 zero-shot NER baseline: typed `DatasetExample` / `EntitySpan` character-span gold set, GLiNER2/legacy GLiNER model loader, nervaluate strict metrics, and JSON result merge in `eval/results/gliner2_baseline_ru.json`; opt-in via `python -m atman.eval.gliner2.baseline --model <hf-model-id>` |
...
| current PR | Sentry `slog` breadcrumb hook mirrored raw memory/user text fields (`content`, `what_happened`, `query`, `anchor`, `anchors`, `text`) to external breadcrumbs/log attributes despite event-level scrubbing | covered (`tests/test_sentry_slog_hook.py::test_sentry_slog_attrs_redacts_memory_and_user_text`, `tests/test_sentry_slog_hook.py::test_sentry_slog_hook_keeps_sensitive_data_out_of_breadcrumb`) |
...
| PR #604 | Skill-manager design gaps: `trigger_router` not wired in runner; draft skills never promoted; behavioral hints only at session end; `inspect-invocations` was a SQL placeholder; auto-pin used lifetime counts; `SkillManagerPort` lived under `atman.skills` | covered (`tests/test_skill_trigger_in_runner.py`, `tests/test_skill_daily_deep_hooks.py::TestDraftPromotion`, store/manager tests for `list_invocations_by_skill` / `count_invocations_in_last_n_sessions`) |
...
## 9. Auto-generated summaries (Living Codemap)

> Sections below are regenerated by `make codemap` (AST-based). Do not edit.

### 9.1 Domain models — quick index
...
### 9.2 Ports — quick index

<!-- codemap:auto:start section="modules-ports" -->
<!-- Updated automatically by `make codemap`. Do not edit. -->
| File | Public classes | Ports |
|------|----------------|-------|
...
<!-- codemap:auto:start section="modules-services" -->
<!-- Updated automatically by `make codemap`. Do not edit. -->
| File | Public classes | Ports |
|------|----------------|-------|
...
### 9.4 Adapters — quick index

<!-- codemap:auto:start section="modules-adapters" -->
<!-- Updated automatically by `make codemap`. Do not edit. -->
| File | Public classes | Ports |
|------|----------------|-------|
...
### 9.5 Port–Adapter matrix

<!-- codemap:auto:start section="port-adapter-matrix" -->
<!-- Updated automatically by `make codemap`. Do not edit. -->
...
<!-- codemap:auto:start section="todos" -->
<!-- Updated automatically by `make codemap`. Do not edit. -->
