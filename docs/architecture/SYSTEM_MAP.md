...
| `observability/sentry_init.py` (HLE-240) | — | canonical `init_observability(level)` / `is_enabled()` / `is_full_mode()`; `ATMAN_OBS_LEVEL` tiers (`off` \| `minimal` \| `debug` \| `verbose` \| `full`); `full` enables `send_default_pii=True`, all prompts, no filtering, streaming AI spans (NOT for production); `off` never imports `sentry_sdk`; levels wire `traces_sampler`, scrubbing, Spotlight, profiling per `docs/features/observability/levels.md` |
...
| `observability/__init__.py` (`init_observability`, `is_enabled`, `is_full_mode`, `ObsLevel`) | — | New unified observability entrypoint (HLE-239 P1.1); reads `ATMAN_OBS_LEVEL` env var (off/minimal/debug/verbose/full); true no-op on "off" (no sentry_sdk import); `minimal` enables sampling + PII scrubbing; `debug` adds Spotlight + profiles; `verbose` adds stack traces and local variables; `full` captures all PII/prompts/NLP data — NOT for production |
