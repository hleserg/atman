...
| `observability/sentry_init.py` (HLE-240) | — | canonical `init_observability(level)` / `is_enabled()`; `ATMAN_OBS_LEVEL` tiers (`off` \| `minimal` \| `debug` \| `verbose`); `off` never imports `sentry_sdk`; levels wire `traces_sampler`, scrubbing, Spotlight, profiling per `docs/observability/levels.md` |
