...
Тесты для PostgreSQL и Ollama пропускаются, если не задана соответствующая переменная окружения или маркер.

## Маркеры тестов

| Маркер | Смысл | Условие пропуска |
|--------|-------|-----------------|
| *(нет)* | Юнит-тест — без внешних зависимостей | Всегда запускается |
| `slow` | Долго выполняемый тест | `-m "not slow"` |
| `integration` | Несколько реальных компонентов без моков | Требует полного стека |
| `e2e` | Полный end-to-end сценарий | Требует агент + LLM |
| `requires_ollama` | Нужен запущенный экземпляр Ollama | `OLLAMA_HOST` не задан |
...
```bash
# Полный набор (в стиле CI)
uv run pytest tests/ -v --cov=atman --cov-fail-under=90

# Быстро (пропустить медленные тесты)
