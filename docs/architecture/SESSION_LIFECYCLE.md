...
| `record_fact()` | Агент вызывает `record_key_moment` с `fact_refs=[current_fact_id]` чтобы покрасить факт, затем следующий факт |
| `exit_facts()` | Вернуться в главное меню |

После `record_fact()`: факт перемещается из "неокрашенных" в "окрашенные" — при следующем вычислении `unexamined_fact_refs` его там не будет.
