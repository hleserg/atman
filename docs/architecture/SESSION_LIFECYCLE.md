...
### 2.5 Ответы агента в "свободное время"

Когда таймаут срабатывает и агент активен (рефлексирует, смотрит факты) — у него нет пользователя-адресата. Его ответы выводятся в терминал как обычно (`print(result.data)`), помечая что это внутренняя активность:

```
[свободное время] {ответ агента}
```

Если агент решил поспать — его финальный ответ тоже выводится перед закрытием сессии.
