...
def test_ai_chat_span_op(mock_sentry):
    ...
    with ai_chat_span("anthropic", "claude-opus-4-7") as entered:
...
def test_ai_chat_span_custom_op(mock_sentry):
    mock_start, span = mock_sentry
    with ai_chat_span("openai", "gpt-4o", op_name="stream") as entered:
        assert entered is span
...
def test_ai_embeddings_span(mock_sentry):
    mock_start, span = mock_sentry
    with ai_embeddings_span("bge", "BAAI/bge-m3") as entered:
        assert entered is span
...
def test_ai_rerank_span(mock_sentry):
    mock_start, span = mock_sentry
    with ai_rerank_span("bge", "BAAI/bge-reranker-v2-m3", docs_in=20, top_n=5) as entered:
        assert entered is span
...
def test_memory_span(mock_sentry):
    mock_start, span = mock_sentry
    with memory_span("recall", "facts", agent_id="agent-1") as entered:
        assert entered is span
...
def test_db_span_postgresql(mock_sentry):
    mock_start, span = mock_sentry
    with db_span("postgresql", "SELECT", collection="facts") as entered:
        assert entered is span
...
def test_db_span_no_collection(mock_sentry):
    ...
    with db_span("qdrant", "search") as entered:
        assert entered is span
...
def test_cron_span(mock_sentry):
    mock_start, span = mock_sentry
    with cron_span("atman-maintenance") as entered:
        assert entered is span
...
def test_pipeline_span_uses_name_not_description(mock_sentry):
    ...
    with pipeline_span("atman.ner", "entity detection") as entered:
        assert entered is _span
