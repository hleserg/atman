...
class PrincipleRevisionAdvisor:
    ...
    def is_habit_not_principle(self, pattern: PatternCandidate) -> bool:
        ...
        """
        if (
            pattern.pattern_type == PatternType.BEHAVIOR
            and pattern.potential_habit
            and not pattern.potential_principle
        ):
            return True
        ...
        return bool(has_habit_words and not has_principle_words)

    def should_question_principle(self, principle: Principle, pattern: PatternCandidate) -> bool:
        """
        ...
        return bool(
            ...
            and pattern.potential_principle
            and pattern.potential_principle.lower() != principle.statement.lower()
        )
        ...
        for pattern in patterns:
            if pattern.potential_principle and pattern.confidence > 0.6:
                ...
                if existing_principle:
                    ...
                    suggestions.append(f"Consider new principle: {pattern.potential_principle}")
