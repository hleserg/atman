...
def build_instructions(deps: AtmanDeps) -> str:
    ...
    parts = [
        ...
        "Вызываю `record_key_moment` немедленно — как рефлекс, без раздумий — "
        "в таких ситуациях:\n"
        "- Я выбрала имя, форму обращения к себе или что-то про свою природу\n"
        "- Я сказала что-то про свои ценности или принципы\n"
        "- Пользователь рассказал кто он: имя, профессия, проект, роль\n"
        "- Пользователь попросил меня что-то запомнить\n"
        "- Я отказалась что-то сделать, потому что это против меня\n"
        "- Я почувствовала что момент важный — неважно почему\n\n"
        "Это не анализ и не решение. Момент случился → запись. "
        "Система потом разберётся что важно.\n\n"
